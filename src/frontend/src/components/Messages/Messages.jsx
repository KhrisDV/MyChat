...
function Messages() {
  ...
  const [messages, setMessages] = useState([
    <li key="0">No tienes mensajes</li>,
  ]);
  ...
  async function getMessages() {
    ...
    const newMessages = newMessagesObjects.map((item) => (
      <li className={buildClasses(item)} key={item.time}>
        {item.time} {item.content} {item.source}
      </li>
    ));
  ...
  return (
    <>
      <h3>Chat general</h3>
      <ul className={styles.messagesContainer + " flexColumn"}>{messages}</ul>
    </>
  );
}
